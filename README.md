...
- The reactive editor keeps the header, preview, mask selection, and QR details in one client boundary. Canvas, marker, result, mask preview, and icon gallery remain lazy where useful; the preview stays above the fold and the mobile side panels use native dialogs rather than duplicated settings DOM.
