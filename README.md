...
After content is committed, QR details, placement, and mask changes update the preview automatically. With empty committed content, selecting a mask highlights its region on the poster without preparing a QR. Enter and commit text or a URL to add the QR preview; assembly remains disabled until the input is valid. The existing full-resolution assembly/download remains an export action, while the editing surface has no Continue/Back step rail. Placement, marker dialogs, artifacts, exact Blob sharing, and the renderer's pixel/verification invariants are unchanged.
