...
2. **Step 2 — Mask or poster.** The editor detects the dense black shape and highlights it in green. If needed, upload an optional same-size **region mask PNG**: white selects the region, black excludes it. Under this step you can also draw the mask from a word in one of the bundled display fonts (see `public/fonts/ATTRIBUTION.md` for authors, sources, and license terms — most are personal-use-only) instead of uploading one: type the word, preview the letterforms, and apply it as the mask.
