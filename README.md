...
码码虎虎 — a web editor for artistic QR posters. Upload a PNG with a solid black region, enter text or a URL, position the QR, and download a full-resolution assembled poster.
...
The visual language pairs the **mahu-QR** mascot (码码虎虎, `public/brand/mahu-tiger.svg`) with the existing Wired-Elements look: the mascot's two flat inks — near-black `#101211` and vermilion `#ff321e` — on warm cream, the hand-drawn multi-corner radii, tilts and wavy underlines, hard offset shadows, and the original Gloria Hallelujah handwriting UI. The mask fonts are separate. The controls are app-specific React components styled in [StyleX](https://stylexjs.com). The browser receives compiled atomic styles for the editor; React-Toastify supplies its own notification styles and uses the editor palette.
...
- Brand assets are static files, not generated routes: `public/brand/mahu-tiger.svg` (the mascot, also the header lockup and empty state), `src/app/icon.svg` (favicon), `src/app/apple-icon.png`, and `src/app/opengraph-image.png` with `opengraph-image.alt.txt`. `public/fonts/ATTRIBUTION.md` records the bundled mask-font licenses.
