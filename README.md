...
After content is committed, QR details, placement, and mask changes update the preview automatically. Selecting a manual mask also reveals the live poster preview before valid content is committed, using a placeholder QR only for that editing preview; assembly remains disabled until the input is valid. The existing full-resolution assembly/download remains an export action, while the editing surface has no Continue/Back step rail. Placement, marker dialogs, artifacts, exact Blob sharing, and the renderer's pixel/verification invariants are unchanged.
...
- The reactive editor keeps the header, preview, mask selection, and QR details in one client boundary. Canvas, marker, result, and icon gallery remain lazy where useful; the preview stays above the fold and the mobile side panels use native dialogs rather than duplicated settings DOM.
