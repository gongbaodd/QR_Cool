...
2. **Step 2 — Select mask.** Set a white canvas width and height (up to 4 megapixels total) and choose **Use blank canvas** for a full-canvas region with no upload, or upload a **Poster PNG** instead. The editor detects the dense black shape and highlights it in green. If needed, upload an optional same-size **region mask PNG**: white selects the region, black excludes it. Under this step you can also draw the mask from a single letter in one of the bundled display fonts (see `public/fonts/ATTRIBUTION.md` for authors, sources, and license terms — most are personal-use-only): the input holds one character only, the font picker is a visual row showing that letter in every font, and **Use letter as mask** applies it.
