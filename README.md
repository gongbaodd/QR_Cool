...
3. **Step 3 — Adjust QR.** Drag the QR, resize a corner, or rotate it with the free rotation handle (no snap; default angle 0). Size snaps to whole QR modules, at least four pixels per module, and the placement includes the normalized QR's two-module source margin. QR generation stays upright — rotation rotates the complete finished plate during placement/compositing, and validation tests the rotated footprint against the selected region. The rotated working frame comes from the exact selected-region bounds, and a frame-coverage guard rejects (in validation and again in assembly) any placement whose plate the frame could not hold, so a rotated plate is never silently clipped. Dragging, resizing, and recentring preserve the angle. Arrow keys move one pixel and Shift+arrow moves ten; on touch screens use the direction buttons or drag. Pattern settings sit beside the canvas, and the step rail (**Step 1 Input text**) takes you back to step 1.
