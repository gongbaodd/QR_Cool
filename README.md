...
2. **Step 2 — Mask Search.** Search icons or use a letter: the input holds up to 10 characters, the first character drives three letter tiles (including **blank** for full canvas, see `public/fonts/ATTRIBUTION.md`); when the query is 2+ characters, the last nine tiles show icons from `https://icons.grida.co/api/search?q=` (front three keep the first-letter rule). The grid is fixed 3×4 with the last cell **more** — clicking it shows all results in the preview area, and picking one re-centers the 12 sidebar options to the nearest icons. Selecting any tile draws its white-on-black mask on a fixed 1000×1000 canvas. No size or upload is needed.
