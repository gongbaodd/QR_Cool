...
| `--assemble` | Offline end-to-end poster: aligned texture, rounded cut with a black mask border, composite, rounded QR plate holding the code at a one-module margin, geometry checks, and a schema-6 report. |
...
2. Cleans the region mask (disc closing then opening at one module), traces it, and fillets it with `--cut-radius` (default: two modules) and `--cut-smooth` (default: one module). Cleaning fills the pixel pinholes and drops the jags the detector leaves behind, so the outline reads round instead of a staircase. The shape is the detected or supplied region minus the QR plate: the window is appended to the traced path as a rounded-rectangle subpath and the even-odd fill turns it into the one hole in the cut.
...
5. Fills the hole with white — the **QR plate** — at the same coverage as the cut edge, so the texture and the plate meet along one antialiased seam, and copies the normalized QR **with a single quiet-zone module kept**: the 39-module plate — the 185px code grid plus a 5px light margin — sits at 254,206, so the texture stops one module short of the modules instead of wrapping the code in a wide white square. The plate's corners are rounded by the same `--cut-radius` (two modules by default), so the texture wraps the rounding instead of being sliced by a square: the four corner arcs hand 56 window pixels back to the texture, and `--cut-radius 0` writes the straight square window back. The plate carries no black band. The profile specifies a two-module margin, so the run does not claim a scannable result; see the decode table below for what a margin that narrow actually does.

With the bundled poster and QR the run reports the region at 125,497px, a 205px QR at 5px/module (box 249,201) drawn on a 195×195 plate with 10px corners at 254,206, a version-30 texture at seed 1 cropped at 9/71 so its lattice matches, and a single cleaned, filleted loop of 38 vertices covering 127,383px² with a 20px border. The cut path is that loop minus the 37,939px² plate, so the net area lands at 89,443px², and it carries one hole. The raw mask traces to 1,932 vertices; cleaning and the two-module fillet are what collapse it to 38.
...
| Plate | Full size | 50% | JPEG-80 |
| --- | --- | --- | --- |
| Square window, 2 modules (source profile) | decodes | decodes | decodes |
| Square window, 1 module (earlier behaviour) | decodes | decodes | decodes |
| Rounded plate, 1 module, 2-module corners (`--assemble` default) | decodes | decodes | decodes |
| Square window, 0 modules (texture runs to the modules) | fails | fails | fails |

One module is enough for the local decoder on this clean render, and the rounding costs nothing at these scales, but it is half of what the QR profile asks for, so the run still does not verify the poster and `phoneScan` stays `untested`: a phone sees perspective, blur and glare that this decoder does not. The report adds `qrPlateCorners` to the checks, so the corner pixels the rounding hands back to the texture are verified rather than assumed, and the corner texture pixel count is recorded for every radius: 0 at `--cut-radius 0`, 4px at 4, 12 at 5, and 56 at the 10px default. Cutting the margin entirely (the earlier behaviour) produced a poster that decoded nowhere.

Artifacts: `poster.png`, `pattern-cut.png` (the composited cut layer, clipped to the region, border included, with the plate as a transparent hole), `pattern-cut.svg` (the same cut with the border as a stroked vector path and the plate as a rounded hole subpath), `region-mask.png`, `qr.png` (the normalized 205px QR, kept as evidence for the quiet-zone rebuild), and `report.json`.

The schema-6 report records the input hashes and sizes, the region block, the QR metadata with `quietZoneSource: "added"` when a margin was rebuilt plus the `overlay` block describing exactly what was composited (one quiet-zone module kept, the 5/5/195 crop and its 254,206 position), the placement, the full pattern block (seed, alphabet, text length and hash, version, module counts, pitch, removed marker types, refill count, code size, crop, `alignment` phase), the cut block (radius, smoothing, cleanup radius, border width/colour/side, keep rule, speck threshold, edge blend rule), the `qrPlate` block (margin modules, corner radius, `rounded-rect` path, box, and the corner texture pixel count), the shape statistics — the loop counts and vertices of the traced region, plus the plate as the one hole and the net area — every artifact hash, the verification checks, `phoneScan: "untested"`, and warnings. Checks are `sourceQr`, `normalizedQr`, `outsideRegionPixels`, `qrPixels`, `qrPlateCorners`, and `alphaPreserved`; the three poster decode checks are listed in `verification.skippedChecks` and a warning says so. Verification failures exit 4.
