...
2. **Step 2 — select mask.** Draw the region from one letter in a bundled display font or choose **blank** for a full-canvas region (see `public/fonts/ATTRIBUTION.md` for authors, sources, and license terms — most are personal-use-only): the input holds one character only, the font picker is a visual row showing that letter in every font (including **blank**) and clicking a font applies it on a fixed 1000×1000 white canvas (visually shown as selected). No size or upload is needed.
