 ...
 */
export function useMaskSelection({ revision, prepared, iconResults, suggestedMask, closeGallery, onUploadMask, dispatch }: MaskSelectionOptions): MaskSelection {
  ...
  async function applyTextMask(fontId = maskFontId) {
    ...
    if (blank) {
      ...
      canvas.toBlob(blob => { if (blob) onUploadMask(new File([blob], TEXT_MASK_FILENAME, { type: 'image/png' })) }, 'image/png')
      return
    }
    ...
    try {
      ...
      dispatch({ type: 'error', revision, message: `Could not load the ${entry.label} mask font. Please retry.`, field: 'mask' })
      setMaskBusy(false)
      return
    }
    ...
    canvas.toBlob(blob => { setMaskBusy(false); if (blob) onUploadMask(new File([blob], TEXT_MASK_FILENAME, { type: 'image/png' })) }, 'image/png')
  }
  ...
  useEffect(() => {
    ...
    if (isIconMode) {
      ...
      const download = item?.download ?? item?.variants[0]?.download
      if (!item || !download || !prepared) { setMaskFit(null); return }
  ...
  return {
    text: maskText, setText: setMaskText, fontId: maskFontId, font: maskFont, selectedIconId, effectiveMask,
    isBlank, isIconMode, busy: maskBusy, fit: maskFit, tooSmall: maskTooSmall, selectFont, selectIcon,
  }
}
