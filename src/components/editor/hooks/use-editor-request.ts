...
export function useEditorRequest({ state, dispatch, poster, mask }: EditorRequestOptions): EditorRequest {
  ...
  useEffect(() => { if (!poster) return; const timer = setTimeout(() => { void request('prepare') }, 450); return () => { clearTimeout(timer); abort.current?.abort() } }, [state.revision, poster, mask]) // eslint-disable-line react-hooks/exhaustive-deps -- Responses never increment the revision.
