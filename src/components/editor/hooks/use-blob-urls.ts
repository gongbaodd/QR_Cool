...
function blobFromBase64(value: string, type: string) {
...
export function useBlobUrls(values: Record<string, string>) {
  ...
  useEffect(() => {
    const next = Object.fromEntries(
      Object.entries(JSON.parse(key) as Record<string, string>).map(([name, bytes]) => [
        name,
        URL.createObjectURL(
          blobFromBase64(
            bytes,
            name.endsWith('.svg') ? 'image/svg+xml' : name.endsWith('.json') ? 'application/json' : 'image/png',
          ),
        ),
      ]),
    )
