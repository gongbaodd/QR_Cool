...
export default function Editor() {
  ...
  return <main>
    ...
    {step === 1 ? <div className="workspace-centered"><aside>
      ...
      {state.showingResult && state.result ? <div className="result"><img src={artifacts['poster.png']} alt="Assembled artistic QR poster" /><div className="result-actions"><a className="primary" href={artifacts['poster.png']} download="poster.png">Download poster.png</a><button onClick={() => dispatch({ type: 'view', result: false })}>Return to editing</button></div><p className="scan-note">Artistic margins can affect scanning. Test the downloaded poster with your phone.</p><details><summary>Artifacts & verification</summary><div className="downloads">{Object.keys(state.result.artifacts).filter(name => name !== 'poster.png').map(name => <a key={name} href={artifacts[name]} download={name}>{name}</a>)}</div></details></div> : step === 2 ? <div style={{ border: '2.5px solid var(--ink)', borderRadius: 'var(--sketch-card)', overflow: 'hidden', background: 'var(--paper)', boxShadow: 'var(--shadow-lg)', padding: 18, display: 'flex', justifyContent: 'center' }}><canvas ref={maskPreview} width={600} height={600} aria-label="Mask text preview" style={{ width: '100%', maxWidth: 560, aspectRatio: '1 / 1', borderRadius: 12, background: 'black' }} /></div> : state.prepared && state.placement && posterUrl ? <Canvas mask={previews['region.png'] ?? ''} poster={posterUrl} overlay={previews['mask.png'] ?? ''} qr={previews['qr.png'] ?? ''} width={state.prepared.width} height={state.prepared.height} placement={state.placement} modules={state.prepared.qrMetadata.totalModules} onChange={move} invalid={!!state.error} /> : <div className="empty"><div className="empty-icon">＋</div><h3>Your poster goes here</h3><p>Pick a canvas and place your QR.</p></div>}
