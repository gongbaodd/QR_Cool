...
export default function Editor() {
  ...
  useEffect(() => { iconSearch.closeGallery() }, [maskSelection.text]) // eslint-disable-line react-hooks/exhaustive-deps
  // Entering step 2 re-derives the control from the input and starts on the mask preview.
  useEffect(() => { iconSearch.closeGallery() }, [step]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  function edit(patch: Parameters<typeof reducer>[1] & { type: 'edit' }) { cancel(); if (patch.patch.content !== undefined) setTextConfirmed(false); dispatch(patch) }
  ...
  function handleSearch() { void iconSearch.handleSearchClick(searchQuery, maskSelection.busy) }
  ...
  return <main>
    ...
    /> : <div className={step === 2 ? "workspace workspace-step2" : "workspace"}><aside>
      ...
      {step === 3 && <StepAdjust
        ...
        onAssemble={() => { void request('assemble'); setStep(4) }}
        onGoto={goto}
      />}
      {step === 4 && <StepGenerate
        state={state}
        ...
        onRetry={() => void request('prepare')}
        onBack={() => { dispatch({ type: 'view', result: false }); goto(3) }}
      />}
      {preparationError && step !== 4 && <PreparationError message={preparationError} onRetry={() => void request('prepare')} />}
    </aside><PreviewPanel
      step={step}
      ...
      gallery={{ query: searchQuery, total: iconSearch.total, items: iconSearch.results, selectedIconId: maskSelection.selectedIconId, onSelect: handleGallerySelect, onClose: iconSearch.closeGallery }}
