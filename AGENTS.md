...
Local TypeScript CLI (`qr-poster`) for artistic QR posters. Five modes: `--dry-run` prepares the layout, `--generate` fills the painted region with Qwen, `--generated-image` recomposes saved artwork, `--pattern-preview` renders an offline marker-free QR texture, `--pattern-cut` cuts that texture to a mask as a filleted SVG path plus a transparent PNG.
...
- `output/` and `dist/` are gitignored run products; reports keep their schema versions (1 dry-run, 2 generation, 3 pattern preview, 4 pattern cut).
