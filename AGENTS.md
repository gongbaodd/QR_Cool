...
- `output/` and `dist/` are gitignored run products; reports keep their schema versions (1 dry-run, 2 generation, 3 pattern preview, 4 pattern cut, 7 assembly).
...
- `--assemble` is the offline end-to-end mode and follows one workflow: QR generator -> module matrix -> region ("B") mask -> safe-area calculation -> draw full modules. It resolves the layout exactly like `--dry-run`, builds the marker-free module matrix, samples the mask on the placed QR's lattice, keeps only the modules the mask covers in full, draws those whole modules over the original pixels, cuts the QR plate out as a module hole, draws the normalized QR inside it, verifies, and writes `poster.png`, `pattern-cut.png`, `pattern-cut.svg`, `region-mask.png`, `qr.png`, and a schema-7 `report.json`.
...
- `--assemble` composites the normalized QR at a light margin of `--qr-margin` modules, one by default: the 39-module plate (37-module code grid plus a 5px margin) sits at 254,206, its window is lattice-aligned, its whole modules are the cut's hole (1,517 after the four corner modules are handed back to the texture, 100 pixels, or all 1,521 with `--cut-radius 0`), no dark band follows it, and every drawn cell survives into the poster untouched. The band around the code is a row of whole cells, so it is quantized by the pitch: one module is the tightest module-level margin, and zero (a hole at the code grid) fails to decode at every scale, which is why zero is rejected. A fractional `--qr-margin` (0.2 = 1px here) is the pixel-tight opt-in: the window is painted at pixel precision, which trims that many pixels off each texture cell along the plate edge (73 cells at 1px), records `qrPlate.path: "pixel-window"` and warns. The margin is half of the profile's two modules, so the poster is deliberately not decode-verified: `sourceQr`, `normalizedQr`, `outsideRegionPixels`, `qrPixels`, `qrPlateCorners`, `moduleCut`, and `alphaPreserved` run, `poster`/`posterHalfScale`/`posterJpeg80` are listed in `verification.skippedChecks`, `phoneScan` stays `untested`, and warnings record the trade. Measured on the bundled poster: the default decodes at all three scales, `--qr-margin 0.2` at full size and JPEG-80 only, and a whole-module hole at the code grid at none.
...
- `--seed` (shared with `--pattern-preview`) makes the texture reproducible; the same seed reproduces identical bytes. `--qr-margin` (1 module by default) moves only the plate's light margin; a whole number keeps the cut module-level, a fraction trades that for a pixel-tight band.
