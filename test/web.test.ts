...
describe('buffer engine migration', () => {
  ...
  it('preserves whitespace and refuses to move an invalid manual placement', async () => {
    ...
    await expect(
      assembleFromBuffers({ posterBytes, content, placement: { x: 0, y: 0, size: 145 }, ...settings }),
    ).rejects.toThrow()
  })
...
describe('stateless routes', () => {
  it('prepares, assembles, and returns the exact engine PNG', async () => {
    ...
    const r = await handleEditorRequest(
      request({ revision: 8, content, settings, placement: body.placement }),
      'assemble',
    )
  ...
  it('rejects corrupt PNG, wrong mask size, and missing region', async () => {
    const white = await sharp({ create: { width: 30, height: 30, channels: 4, background: 'white' } })
      .png()
      .toBuffer()
    for (const req of [
      request({ revision: 0, content, settings }, Buffer.from('bad')),
      request({ revision: 0, content, settings }, posterBytes, white),
      request({ revision: 0, content, settings }, white),
    ]) {
      const r = await handleEditorRequest(req, 'prepare')
      expect(r.status).toBe(422)
    }
  })
  it('bounds ingress before parsing, including chunked bodies', async () => {
    ...
    const r2 = await handleEditorRequest(
      new Request('http://localhost', {
        method: 'POST',
...
it('finds a smaller assembly-valid automatic placement on a small poster', async () => {
  const small = await sharp({ create: { width: 240, height: 240, channels: 4, background: 'white' } })
    .png()
    .toBuffer()
  const mask = await sharp({ create: { width: 240, height: 240, channels: 4, background: 'black' } })
    .composite([
      {
...
it('revalidates longer content around the previous center and never exports an obsolete QR', async () => {
  ...
  const changed = await prepareEditor({
    posterBytes,
...
it('rejects pixel-limit and actual encoder capacity violations', async () => {
  const huge = await sharp({ create: { width: 2001, height: 2000, channels: 3, background: 'black' } })
    .png()
    .toBuffer()
