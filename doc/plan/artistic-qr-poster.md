...
4. The normalized QR keeps **one** quiet-zone module: the 39-module plate (222px code grid plus a 6px margin) is copied verbatim from the normalized image's 6,6 corner to 224,187, its four corner modules excepted. `--qr-margin` can make that band a fraction of a module instead, which is painted at pixel precision — see "A fractional QR margin" below. The poster is then verified with geometry only — `sourceQr`, `normalizedQr`, `outsideRegionPixels`, `qrPixels`, `qrPlateCorners`, `moduleCut`, `alphaPreserved` — while `poster`, `posterHalfScale`, and `posterJpeg80` are recorded in `verification.skippedChecks`.
...
Measured: region 125,497px (auto), QR version 5 at 6px/module in the maximum box 218,181,246 drawn on the 234×234 plate at 224,187 (1 quiet-zone module kept, crop 6/6/234), pattern version 24 at seed 1 cropped at 4/65 with a residual alignment phase of 0/0, a 115×95 lattice with 3,284 safe modules, 367 dropped partial modules (7,273 region pixels) and 1,767 drawn modules of which 1,008 are rim and 759 texture, all seven checks passing. Artifacts are `poster.png`, `pattern-cut.png`, `pattern-cut.svg`, `region-mask.png`, `qr.png`, and a schema-7 `report.json`. Pixel counts remain zero for transparency, alpha differences against the source poster, changes outside the region or outside drawn modules, trimmed drawn cells at the default margin, and plate-window mismatches.

### Why the margin was one module, and what it costs

The tight QR is the point of this iteration: the white square around the code made it read as a pasted block instead of part of the artwork. The first cut took the whole quiet zone, which made the poster unscannable at every scale. Keeping one module (6px on the bundled version-5 fixture, exactly one texture cell) keeps the code separated from the texture without the wide white square; a fractional margin remains an explicit opt-in.
...
One module of white around a 6px code can still read as a pasted border, so `--qr-margin` accepts a fraction; the plate then has to be painted at pixel precision, and that is a deliberate trade rather than a free win:

- The light band around the code is a row of whole modules, so it is quantized by the pitch: one module is the tightest margin that is also module-level, and zero (a module-aligned hole at the code grid) removes the quiet zone and fails to decode at every scale, which is why `--qr-margin 0` is rejected rather than rounded away. That is why the default stays at one module: at a whole-module margin the plate window is lattice-aligned, the hole is whole modules, and every drawn cell survives into the poster untouched (measured: 1,767 of 1,767).
- A fractional margin, e.g. `--qr-margin 0.2` (1px at this pitch), sets `marginPixels = max(1, round(margin × pitch))` and paints the plate window at pixel precision. The window is the placement box inset by `QUIET_ZONE_MODULES × pitch − marginPixels` — 224×224 at 229,192, an 11/11 crop of the normalized QR — and `computePlateModules()` drops the 1,369 modules it covers in full while the modules it partly covers stay drawn and are painted over by the plate. That trims the plate's margin off the neighbouring cells, which the report warns about by name.
...
Measured on the bundled fixtures at the default: 234px window, 1,517-module hole, 144 corner texture pixels, 1,767 drawn modules (759 texture + 1,008 rim), no trimmed cell, and local decoding at all three scales. `--qr-margin 0.2` gives a 224px pixel window, a 1,369-module hole, 4 corner pixels, and 1,915 drawn modules with only the plate-edge cells trimmed. `test/assemble.test.ts` asserts the default, the whole-cell guarantee and the 0.2 case, and `test/module-cut.test.ts` covers the off-lattice window.
...
Measured on the bundled fixtures (`--seed 1`): region 125,497px, QR version 5 at 6px/module in box 218,181,246, a 115×95 lattice at phase 2/1, 3,284 safe modules, 367 dropped partial modules (7,273 region pixels kept as artwork), 1,517 plate hole modules, 1,767 drawn modules (1,008 rim + 759 texture, 63,612px), 144 corner texture pixels, and all seven checks passing. The unfilled mask edge is the price of never slicing a cell, while the maximum QR fills most of the central region.
