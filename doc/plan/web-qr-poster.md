...
1. **Upload:** a labeled file input accepts a poster PNG and displays its dimensions. Detect the dense black shape using the current detector. If no suitable region is found, show an actionable error. If the selection is wrong, allow an optional same-size region-mask PNG using an explicitly labeled white-selects-region convention, matching the existing engine. Keep this distinct from the black shape embedded in the poster. A **Use blank canvas** button generates a white 1000 × 1000 poster plus a full-white mask in the browser (no upload; a pure white sheet has no black shape for the detector, so the mask carries the region). A letter-mask control renders one suggested character in a bundled display font (`public/fonts`, white on black at poster size; authors, sources, and license terms in `public/fonts/ATTRIBUTION.md`, most personal-use-only) into an equivalent mask file, so no upload is needed for that path either. The letter is suggested from website-like content (first host letter without scheme/`www.`) and plain text starts blank; the font picker is a visual row rendering that letter in every font and the input holds one character only.
2. **Content:** accept one nonblank line of text or a URL as step 1, before mask selection. Preserve the exact entered value, including surrounding spaces; use trimming only to detect an empty value. Report newline and QR-capacity errors beside the field. A URL is encoded as text and is never fetched.
