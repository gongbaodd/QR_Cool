...
3. **Position:** generate the QR and suggest an assembly-valid position. Let users drag it, resize with corner handles, and nudge it with the arrow keys or the on-screen direction buttons. Keep the QR square, unrotated, and entirely inside the selected region. Placement always stays sendable: drags and nudges clamp to the poster origin, and a content or error-correction change that alters the module count re-centres the previous box on the new size inside the canvas before it is revalidated. An invalid placement is flagged in the sidebar and left where it is, never silently moved.
