...
Keyboard navigation follows visible header order: the simple URL/Text/Phone field is first, then content-type choices, examples, and Home; structured content types place **Edit details** first. Editor controls share a 3px dotted vermilion `:focus-visible` outline with a 3px offset. Keyboard-focused pixel-style and marker-shape radios outline their visible card, and SVG marker targets show the dotted indicator at the marker. Native dialogs and mobile drawers contain Tab and Shift+Tab, dismiss with Escape, and restore focus to their opener.
