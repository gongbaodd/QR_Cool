...
Keyboard navigation starts with the simple URL/Text/Phone field, then content-type choices, examples, and Home; structured content types place **Edit details** first. The Home logo remains at the left side of the header. Editor controls share a 3px dotted vermilion `:focus-visible` outline with a 3px offset. Keyboard-focused pixel-style and marker-shape radios outline their visible card, and SVG marker targets show the dotted indicator at the marker. Native dialogs and mobile drawers contain Tab and Shift+Tab, dismiss with Escape, and restore focus to their opener.
