...
The shipped editor no longer uses the four-step navigation model. Its header owns a controlled Text or URL field; valid draft input commits after a short debounce, while Enter commits immediately. The committed value drives automatic mask derivation and debounced worker preparation. Empty committed input shows the selected region highlighted on the poster without preparing a QR; if a committed QR cannot fit inside the selected region, the preview falls back to the highlighted region without a QR. A persistent toolbar sits flush against the top of the preview canvas throughout editing, with **Fill region** on the left and **Add Rim** on the right in either editing view; the taller preview container handles scrolling and pins the toolbar at its top edge. The selected region remains highlighted in the preview. Activating **Fill region** shows a persistent toast with instructions until fill mode exits. Fill accepts enclosed source-mask areas, including from the QR view's poster background, and Escape exits fill mode. The updated mask is used when the preview is prepared again. Non-empty values derive a letter in the Fathead font, and plain text uses its first ASCII letter or digit. Assembly requires valid input.
