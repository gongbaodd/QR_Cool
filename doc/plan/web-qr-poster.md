...
2. **select mask:** no size or poster/mask upload. Directly use a mask letter: a fixed 1000×1000 white canvas is generated in the browser and the region is drawn from one character in a bundled display font (`public/fonts`, white on black at poster size; authors, sources, and license terms in `public/fonts/ATTRIBUTION.md`, most personal-use-only) plus a **blank** option for the full canvas. The letter is suggested from website-like content (first host letter without scheme/`www.`) and plain text starts blank; the font picker is a visual row rendering that letter in every font (including blank) and the input holds one character only.
