...
Keep seed, pixel style (`square`, `rounded`, `dot` matching `qrcode.antfu.me`), fixed 1-module finder margin, plate corner treatment, rim thickness 0–5, and rounded rim (antialiased) in advanced settings. Marker styling lives in small dialogs opened from hovered/found markers on the canvas: the three 7×7 finder markers share one finder dialog (marker pixel style `square`/`round`, marker shape `square`/`round`/`octagon`, marker inner `square`/`round`/`plus`/`diamond`) and the bottom-right 5×5 alignment marker (version ≥ 2 only) opens the sub marker dialog (`square`/`circle`). Generate the seed once per new poster and retain it across edits; an explicit “New pattern” action changes it. Do not present the old numeric cut-radius control as general silhouette rounding: in assembly it only chooses the marker-corner treatment. Step 1 offers example QR buttons for quick content selection.
...
- The editor sidebar/preview shells load eagerly; the step-specific panels are deferred with `next/dynamic` in the client components: `MaskPreviewCanvas` (with `ssr: false` and a stable-size 1:1 placeholder so no layout shift), `PatternSettings`, `MarkerDialog`, and `ResultPanel` in `PreviewPanel.tsx`, plus `IconGallery` in `Editor.tsx`. Phase-specific `stepWarmers` fetch the next step's chunks once the current step is open (step 1 warms only after the content passes validation), so transitions only swap already-loaded modules and first paint on step 1 downloads none of them.
