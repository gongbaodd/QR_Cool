...
The shipped editor no longer uses the four-step navigation model. Its header owns a controlled draft Text or URL field and a Generate submit that commits valid input; draft typing does not touch the worker or current preview. The committed value drives automatic mask derivation and debounced worker preparation. Empty committed input shows the selected region highlighted on the poster without preparing a QR; if a committed QR cannot fit inside the selected region, the preview falls back to the highlighted region without a QR. A persistent toolbar stays above the changing preview throughout editing, with **Fill region** and **Add Rim** available in either editing view. The selected region remains highlighted in the preview. Fill accepts enclosed source-mask areas, including from the QR view's poster background, and Escape exits fill mode. The updated mask is used when the preview is prepared again. Non-empty values derive a letter in the Fathead font, and plain text uses its first ASCII letter or digit. Assembly requires valid input.
