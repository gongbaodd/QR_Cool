...
2. **Mask Search:** no size or poster/mask upload. The mask comes from a 10-character search field (`TEXT_MASK_MAX_LENGTH=10`) whose first character drives three letter tiles (including **blank** for full canvas, fonts in `public/fonts` white on black at full poster height; see `public/fonts/ATTRIBUTION.md`). When the query is 2+ characters, the 3×4 grid shows those three letter tiles plus eight icons from `GET /api/icons?q=` (proxied to `https://icons.grida.co/api/search?q=`) and a final **more** tile; clicking **more** renders all results in the preview area and picking one re-centers the sidebar’s 12 options to the nearest icons (`nearestWindow`). The front three tiles keep the first-letter rule. Clicking any tile draws its white-on-black mask on the fixed 1000×1000 canvas (letters via `drawTextMask`, icons via `drawIconMask` recolored to white) with no separate size or apply button.
