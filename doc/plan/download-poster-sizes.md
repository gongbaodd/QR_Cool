...
5. Compute preset intermediate choices (for example, near 50% and 75%) by choosing the nearest distinct *verified or validation-eligible integer pitches*. Show their exact dimensions and target module pitch; omit duplicate or impossible choices. Keep the original option regardless of the candidate search outcome.
...
- Keep the preview and download link tied to the exact same Blob for the *active* size. Default to Original. On selection, keep the current preview and its download link visible until the new variant is ready, then switch both together. Keep the existing phone-scan warning beside the download.
